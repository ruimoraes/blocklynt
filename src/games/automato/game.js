...
const VELOCIDADE_EXECUCAO = 100;

const inicializarApiDoInterprete = (interpreter, escopoGlobal, cena) => {
  const criarWrapperAcao = (nomeFuncao) => {
    const wrapper = (callback) => {
      cena[nomeFuncao]();
      setTimeout(callback, VELOCIDADE_ANIMACAO / 2);
    };
    interpreter.setProperty(escopoGlobal, nomeFuncao, interpreter.createAsyncFunction(wrapper));
  };

  const criarWrapperCondicao = (nomeFuncao) => {
    const wrapper = (...args) => cena[nomeFuncao](...args);
    interpreter.setProperty(escopoGlobal, nomeFuncao, interpreter.createNativeFunction(wrapper));
  };

  const wrapperHighlight = function (id) {
    id = String(id || '');
    cena.highlightBlock(id);
  };
  ...
  criarWrapperAcao('moverParaFrente');
  criarWrapperAcao('virarEsquerda');
  criarWrapperAcao('virarDireita');
  criarWrapperCondicao('chegouNoAlvo');
  criarWrapperCondicao('haCaminho');
};
...
export const createGame = (parentElement, configFaseAtual) => ({
  ...
  scene: {
    ...
    create() {
      ...
      // Função para normalizar coordenadas (0 = parede/fora, 1 = caminho)
      const normalize = (x, y) => {
      ...
      // Animação de queda (quando cai no buraco)
      this.anims.create({
        key: 'pegman_fall',
      ...
      for (let y = 0; y < this.mapa.length; y++) {
        for (let x = 0; x < this.mapa[y].length; x++) {
          let tileShape = normalize(x, y) +     // Centro
          ...
          // Obter coordenadas do tile no tileset
          const [tileX, tileY] = TILE_SHAPES[tileShape];
          // Converter para índice linear
          const frameIndex = tileY * 5 + tileX;

          // Criar sprite do tile
      ...
      // Renderizar marker (posição final)
      for (let y = 0; y < this.mapa.length; y++) {
        for (let x = 0; x < this.mapa[y].length; x++) {
          const valorTile = this.mapa[y][x];
          if (valorTile === 3) {
            const markerImg = this.add.image(
              x * TAMANHO_TILE + TAMANHO_TILE / 2,
              y * TAMANHO_TILE + TAMANHO_TILE / 2 - 10,
              'marker'
            );
      ...
      this.animarFalha = () => {
        // Animação de bounce como no código original
      ...
      // Animações de rotação do Oeste (sprite 12)
      this.anims.create({
        key: 'oeste_para_sul',
      ...
      // Animações de rotação do Leste (sprite 4)
      this.anims.create({
        key: 'leste_para_sul',
      ...
      // Animações de rotação do Sul (sprite 8)
      this.anims.create({
        key: 'sul_para_oeste',
      ...
      this.animarRotacao = (novaDirecao) => {
        ...
        // Mapear direções: NORTE=0, LESTE=1, SUL=2, OESTE=3
        ...
        if (this.anims.exists(chaveAnimacao)) {
          ...
          // Listener para quando terminar a rotação
          ...
          // Se não existe animação direta, usar a visual diretamente
      ...
      this.chegouNoAlvo = () => {
        return this.posicaoJogador.x === this.posicaoFinal.x && this.posicaoJogador.y === this.posicaoFinal.y;
      };
      ...
      const executarCodigo = (codigo, workspace) => {
        ...
        const meuInterprete = new Interpreter(
          codigo,
          (interpreter, escopo) => inicializarApiDoInterprete(interpreter, escopo, this)
        );
        ...
        const proximoPasso = () => {
          ...
          try {
            hasMoreCode = meuInterprete.step();
          } catch (erro) {
            console.error("Erro na execução do intérprete:", erro);
            this.resultadoJogada = 'falha';
            checarSucesso();
            return;
          }
      ...
      this.anims.create({
        key: 'pegman_celebrate_1',
        frames: [{ key: 'pegman', frame: 16 }],
        frameRate: 1
      });

      this.anims.create({
        key: 'pegman_celebrate_2',
        frames: [{ key: 'pegman', frame: 17 }],
        frameRate: 1
      });

      this.anims.create({
        key: 'pegman_celebrate_3',
        frames: [{ key: 'pegman', frame: 18 }],
        frameRate: 1
      });
